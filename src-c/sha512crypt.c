...
/* 2013-11-17, Pierce Ng: Worked around some Gnu-isms to get this working
   on OSX and FreeBSD. */
...
sha512_finish_ctx (struct sha512_ctx *ctx, void *resbuf)
{
  ...
  *(uint64_t *) &ctx->buffer[bytes + pad + 8] = SWAP512 (ctx->total[0] << 3);
  *(uint64_t *) &ctx->buffer[bytes + pad] = SWAP512 ((ctx->total[1] << 3) |
						  (ctx->total[0] >> 61));
  ...
  for (unsigned int i = 0; i < 8; ++i)
    ((uint64_t *) resbuf)[i] = SWAP512 (ctx->H[i]);
